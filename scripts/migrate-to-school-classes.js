 ...
 */
async function mapUniversityToSchool(oldData) {
    try {
 ...
 * Build old path
 */
function buildOldPath(oldData, suffix = '') {
 ...
 * Build new path
 */
function buildNewPath(newData, suffix = '') {
 ...
 */
async function migrateCourses(stats) {
            ...
            for (const [collId, collData] of Object.entries(colleges)) {
                ...
                for (const [deptId, deptData] of Object.entries(departments)) {
                    ...
                    for (const [batchId, batchData] of Object.entries(batches)) {
                        const semesters = batchData?.semesters || {};
 ...
 */
async function migrateLectures(stats) {
    ...
    try {
        ...
        if (lecturesSnap.exists()) {
            ...
            for (const [courseId, courseLectures] of Object.entries(lectures)) {
                const lecturesData = courseLectures;
                for (const [lectureId, lectureData] of Object.entries(lecturesData)) {
                    ...
                    if (courseSnap.exists()) {
                        ...
                        await set(ref(db, `edu/sch/classes/${classId}/subjects/${courseId}/lectures/${lectureId}`), {
                            ...lectureData,
                            id: lectureId,
                            courseId,
                            classId,
 ...
 */
async function migrateAssignments(stats) {
    ...
    try {
        ...
        if (assignmentsSnap.exists()) {
            ...
            for (const [assignmentId, assignmentData] of Object.entries(assignments)) {
                ...
                if (data.classId) {
                    await set(ref(db, `edu/sch/classes/${data.classId}/assignments/${assignmentId}`), {
                        ...data,
                        id: assignmentId,
 ...
 */
async function migrateGrades(stats) {
    ...
    try {
        ...
        if (gradesSnap.exists()) {
            ...
            for (const [classId, classGrades] of Object.entries(grades)) {
                const gradesData = classGrades;
                for (const [studentId, studentGrades] of Object.entries(gradesData)) {
                    await set(ref(db, `edu/sch/classes/${classId}/grades/${studentId}`), {
                        ...studentGrades,
 ...
 */
async function migrateSchedules(stats) {
    ...
    try {
        ...
        if (scheduleSnap.exists()) {
            ...
            for (const [batchId, scheduleData] of Object.entries(schedules)) {
                ...
                if (schedule.classId) {
                    await set(ref(db, `edu/sch/classes/${schedule.classId}/schedule`), {
                        ...schedule,
