 ...
 */
async function migrateData(db, oldPath, newPath, description) {
    try {
 ...
 */
async function verifyMigration(db, oldPath, newPath) {
    try {
        const oldRef = ref(db, oldPath);
 ...
 */
async function cleanupOldPaths(db, paths) {
...
async function runMigration() {
    ...
    for (const mapping of MIGRATION_MAP) {
        const result = await migrateData(db, mapping.old, mapping.new, mapping.description);
        if (result.success) {
            results.success++;
        }
        else {
